...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function register($username, $password, $email,$phone, $dfg, $additional_data = array(), $groups = array())
	{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query2 = $this->db->select('phone,email,id')
		                  ->where('phone', $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get('volunteer');
						  	//  print_r($this->db->last_query());exit();
		if(empty($query->result_array())){$user = $query2->row();
			$query = $this->db->select('email, username, email, id, password, active, last_login')
	...
	**/
	...
	////Strat extern web from model----
	 function getword($id1) {
       $this->db->where('party_id',$id1);
 ...
 $url="https://api.msg91.com/api/sendhttp.php?mobiles=".$phone."&authkey=308172AuZ3VC9dU5df325a4&route=1&sender=LALANT&message=".$meg."&country=91";
 $curl = curl_init();
...
curl_setopt_array($curl, array(
  ...
  CURLOPT_SSL_VERIFYPEER => 0,
	 ...
	 public function sendwhatsapp($number,$message){
           $my_apikey = "RKVNVI11IY7WOL69OM06";
           $destination =$number;
